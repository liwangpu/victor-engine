...
export const ons: ReducerTypes<VictorDesignerState, readonly ActionCreator<string, Creator<any[], object>>[]>[] = [
  on(fromAction.setComponentMetadata, (state: VictorDesignerState, { id, configuration }) => {
    const componentTrees = { ...state.componentTrees };
    const componentConfigurations = { ...state.componentConfigurations };
    const maintainBodyComponent = (bodyMetadata: ComponentConfiguration, parentId: string) => {
      if (!componentTrees[bodyMetadata.id]) {
        ...
        componentConfigurations[bodyMetadata.id] = { ...bodyMetadata, body: [] };
      }
    };
    // 删除原组件body已经没有的组件
